...
exports.register = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'User created', user: { fullName: user.fullName, email: user.email } });
  } catch (err) {
...
exports.getProfile = async (req, res) => {
  try {
    const user = await User.findByPk(req.userId, { attributes: ['id', 'fullName', 'email'] });
